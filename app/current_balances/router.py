...
async def get_current_balances(
    ...
    lease_id_search: Optional[str] = Query(
        None, description="Search by lease ID (comma-separated)"
    ),
    driver_name_search: Optional[str] = Query(
        None, description="Search by driver name (comma-separated)"
    ),
    tlc_license_search: Optional[str] = Query(
        None, description="Search by TLC license (comma-separated)"
    ),
    medallion_search: Optional[str] = Query(
        None, description="Search by medallion number (comma-separated)"
    ),
    plate_search: Optional[str] = Query(
        None, description="Search by plate number (comma-separated)"
    ),
    vin_search: Optional[str] = Query(
        None, description="Search by VIN number (comma-separated)"
    ),
    ssn_search: Optional[str] = Query(
        None, description="Search by SSN - full or last 4 digits (comma-separated)"
    ),  # NEW
    # Status filters
    lease_status: Optional[LeaseStatusEnum] = Query(
        None, description="Filter by lease status"
    ),
    driver_status: Optional[DriverStatusEnum] = Query(
        None, description="Filter by driver status"
    ),
    payment_type: Optional[PaymentTypeEnum] = Query(
        None, description="Filter by payment type"
    ),
    dtr_status: Optional[DTRStatusEnum] = Query(
        None, description="Filter by DTR status"
    ),
    # Sorting
...
async def get_lease_balance_detail(
    ...
    try:
        ...
        detail = original_service.get_lease_detail_with_daily_breakdown(
            lease_id=lease_id, week_start=week_start, week_end=week_end
        )
...
async def get_balances_summary(
    ...
    try:
        ...
        dtrs_generated = sum(
            1 for row in balance_rows if row.dtr_status == DTRStatusEnum.GENERATED
        )
        dtrs_not_generated = sum(
            1 for row in balance_rows if row.dtr_status == DTRStatusEnum.NOT_GENERATED
        )

        ach_count = sum(
            1 for row in balance_rows if row.payment_type == PaymentTypeEnum.ACH
        )
        cash_count = sum(
            1 for row in balance_rows if row.payment_type == PaymentTypeEnum.CASH
        )
        ...
        return {
            ...
            "payment_breakdown": {"ach": ach_count, "cash": cash_count},
...
async def export_current_balances(
    ...
    export_format: str = Query(
        "excel", regex="^(excel|csv|pdf|json)$", description="Export format"
    ),
    # Search parameters
    ...
    driver_name_search: Optional[str] = Query(
        None, description="Search by driver name"
    ),
    tlc_license_search: Optional[str] = Query(
        None, description="Search by TLC license"
    ),
    ...
    ssn_search: Optional[str] = Query(
        None, description="Search by SSN (last 4 or full)"
    ),  # NEW
    # Status filters
    ...
    try:
        ...
        if not balance_rows:
            raise ValueError(
                "No balance data available for export with the given filters."
            )
        ...
        logger.error("Error exporting current balances data: %s", e, exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
