...
class UserService:
    ...
    async def authenticate_user(self, login_data: LoginRequest) -> Optional[User]:
        ...
        if is_runner and platform != "mobile":
            logger.warning(
                "Runner role can only login via mobile", email=login_data.email_id
            )
            raise HTTPException(
        ...
        if not is_runner and platform == "mobile":
            logger.warning(
                "Non-runner role cannot login via mobile", email=login_data.email_id
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def change_password(
        self, user: User, password_data: PasswordChangeRequest
    ) -> None:
    ...
    async def request_password_reset(self, request_data: ForgotPasswordRequest):
        ...
        try:
            # Use synchronous email service with S3 template
            success = send_templated_email(
                to_emails=[user.email_address],
                subject_template=settings.password_reset_email_subject_template,
                template_s3_key=settings.password_reset_email_template_key,
                template_data=template_data,
            )

            if not success:
                logger.error(
                    "Failed to send password reset email", email=user.email_address
                )
    ...
    async def reset_password(self, request_data: ResetPasswordRequest):
        ...
        if not user or not user.is_active:
            logger.warning(
                "Password reset attempted for non-existent or inactive user",
                email=email,
            )
            raise HTTPException(
