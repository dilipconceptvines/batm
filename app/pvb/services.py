        ...
        try:
            ...
            for i, row in enumerate(rows):
                try:
                    ...
                    exiting_summaons = self.repo.get_violation_by_summons(summons) if summons else None

                    if exiting_summaons:
                        logger.info(f"Skipping duplicate summons {summons} at row {i+2} in {file_name}")
                        failed_rows_count += 1
                        faild_reasons.append(f"Row {i+2}: Duplicate summons {summons}")
                        continue

                    if summons in inserted_summons:
                        logger.info(f"Skipping duplicate summons {summons} at row {i+2} in {file_name}")
                        failed_rows_count += 1
                        faild_reasons.append(f"Row {i+2}: Duplicate summons {summons}")
                        continue

                    inserted_summons.add(summons)
                    ...
                    violation_data = {
                        ...
                        "fine": fine,
                        "system_entry_date": parse_date(row[8]),
        ...
        for txn_id in transaction_ids:
            try:
                ...
                if violation.status == PVBViolationStatus.IMPORTED:
                    ...
                    new_posting, balance = ledger_service.create_obligation(
                        category=PostingCategory.PVB,
                        amount=total_payable,  # Always use full TP
                    ...
                    audit_trail_service.create_audit_trail(
                        ...
                        meta_data={
                            ...
                            "source_medallion_id": violation.medallion_id,
                            "source_lease_id": old_lease_id,
                            "source_driver_id": old_driver_id,
                            "target_lease_id": new_lease_id,
                            "target_driver_id": new_driver_id,
