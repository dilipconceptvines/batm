...
def list_tlc_violations(
    ...
    from_driver_payable: Optional[float] = Query(None, description="Filter by from driver payable."),
    to_driver_payable: Optional[float] = Query(None, description="Filter by to driver payable."),
    ...
    try:
        violations, total_items = tlc_service.repo.list_violations(
            ...
            from_driver_payable=from_driver_payable,
            to_driver_payable=to_driver_payable,
        ...
        for violation in violations:
            response_items.append(
                TLCViolationListResponse(
                    ...
                    ea_reason = violation.ea_reason if violation.ea_reason else "",
                    ...
                    driver_payable=violation.driver_payable or 0.0,
...
def export_tlc_violations(
    ...
    from_driver_payable: Optional[float] = Query(None),
    to_driver_payable: Optional[float] = Query(None),
    ...
    try:
        violations, _ = tlc_service.repo.list_violations(
            ...
            from_driver_payable=from_driver_payable, to_driver_payable=to_driver_payable,
        ...
        for violation in violations:
            export_data.append({
                ...
                "Driver ID": violation.driver.driver_id if violation.driver and violation.driver.driver_id else "",
                "Medallion": violation.medallion.medallion_number if violation.medallion and violation.medallion.medallion_number else "",
                ...
                "Driver Payable": float(violation.driver_payable) if violation.driver_payable else 0.0,
                "Disposition": violation.disposition.value if violation.disposition else "",
                "Disposition Change Date": violation.disposition_change_date.strftime("%Y-%m-%d") if violation.disposition_change_date else "",
                "Status": violation.status.value if violation.status else "",
                "EA Reason": violation.ea_reason if violation.ea_reason else "",
                "Driver Name": violation.driver.full_name if violation.driver and violation.driver.full_name else "",
                "Driver Email": violation.driver.email_address if violation.driver and violation.driver.email_address else "N/A",
                "Lease ID": violation.lease.lease_id if violation.lease and violation.lease.lease_id else "",
                "Lease Type": violation.lease.lease_type if violation.lease and violation.lease.lease_type else "",
                "Vin": violation.vehicle.vin if violation.vehicle and violation.vehicle.vin else "",
                "Note": violation.note if violation.note else "",
            })
